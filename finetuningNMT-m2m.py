...
from transformers import AutoModelForSeq2SeqLM, DataCollatorForSeq2Seq, Seq2SeqTrainingArguments, Seq2SeqTrainer, AutoTokenizer, EarlyStoppingCallback


def arguments():
    parser = argparse.ArgumentParser(description="Fine-tune NMT models")
    parser.add_argument("-m", "--model", default="facebook/m2m100_418M", help="Folder path: path to model that is going to be fine-tuned.")
    #parser.add_argument("-m", "--model", default="alirezamsh/small100", help="Folder path: path to model that is going to be fine-tuned.")
    parser.add_argument("-d", "--dataset", default="data/", help="Folder path: path to fine-tuning dataset -- must contain a train and dev TSV file.")
    ...
    #parser.add_argument("-src", "--source-language", default="pt", help="Name of the train file.")
    parser.add_argument("-src", "--source-language", default="sq", help="Name of the train file.")
...
dataset_folder = args.dataset + src_lang + "/"

data_files = {"train": args.train_name, "validation": args.dev_name}
raw_datasets = load_dataset(dataset_folder, data_files=data_files)
...
args = Seq2SeqTrainingArguments(
    ...
    learning_rate=2e-5,
    #learning_rate=4e-6, #Testing Small100
